...
def _expected_binfo_dim(binfo_type: str) -> int:
    ...
    if t in ("low", "lowlevel", "low_level"): return beat_channels
...
def collate_lm(batch, LM_T: int = 1024, pad_tok: int = 0, pad_binfo: float = 0.0, binfo_dim: int = beat_channels):
    """
...
# ------------------------------
class Solver:
    def __init__(self, model, vqvae, device, use_wandb=True):
        self.device = device
        self.model = model
        self.vqvae = vqvae
